...
def process_pdfs(pdf_path: Union[str, Path]) -> List[str]:
    ...
    with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
        future_to_file = {executor.submit(parse_pdf, file): file for file in pdf_path.glob("*.pdf")}
        
        for future in tqdm(as_completed(future_to_file), total=total_pdfs, desc="Parsing PDFs"):
