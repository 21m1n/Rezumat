...
def process_all_jobs(model_tuples: List[Tuple[str, RunnableSequence]], job_text: Union[str, List[str]], output_dir: Union[str, Path]):
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        for job_tuple in job_tuples:
            futures.append(executor.submit(two_stage_eval_jd, model_tuples, job_tuple, output_dir))
            
        for future in tqdm(as_completed(futures), total=len(job_tuples), desc="Processing all jobs"):
            try:
...
def process_all_pairs(model_tuples: List[Tuple[str, RunnableSequence]], job_data: List[Tuple[str, str]], cv_data: List[Tuple[str, str]], output_dir: str):
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        for job in job_data:
            for cv in cv_data:
                futures.append(executor.submit(two_stage_eval_cv, model_tuples, job, cv, output_dir))
